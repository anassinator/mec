...
class SimpleKeyTeleop():
    def __init__(self, interface):
        self._interface = interface
        self._pub_cmd = rospy.Publisher('key_vel', Twist)
        self._pub_servo = rospy.Publisher('servo', Int16)
        self._servo_angle = 110

        self._hz = rospy.get_param('~hz', 50)

        self._rates = (0.30, 0.30, 0.30)
        self._forward_rate = rospy.get_param('~forward_rate', self._rates[0])
        self._backward_rate = rospy.get_param('~backward_rate', self._rates[1])
        self._rotation_rate = rospy.get_param('~rotation_rate', self._rates[2])
        self._last_pressed = {}
        self._angular = 0
        self._linear = 0
    ...
    speed_bindings = {
        ord(' '): (0.00, 0.00, 0.00),
        ord('1'): (0.30, 0.30, 0.30),
        ord('2'): (0.35, 0.35, 0.30),
        ord('3'): (0.40, 0.40, 0.35),
        ord('4'): (0.45, 0.45, 0.35),
        ord('5'): (0.50, 0.50, 0.35),
        ord('6'): (0.55, 0.55, 0.40),
        ord('7'): (0.60, 0.60, 0.40),
        ord('8'): (0.70, 0.70, 0.50),
        ord('9'): (0.80, 0.80, 0.60),
        ord('0'): (1.00, 1.00, 1.00)
    }
    ...
    def _key_pressed(self, keycode):
        ...
        elif keycode in self.speed_bindings:
            self._rates = self.speed_bindings[keycode]
            self._forward_rate = self._rates[0]
            self._backward_rate = self._rates[1]
            self._rotation_rate = self._rates[2]
        elif keycode in self.servo:
            self._servo_angle = self.servo[keycode]
            self._pub_servo.publish(self._servo_angle)

    def _publish(self):
        self._interface.clear()
        self._interface.write_line(2, 'Linear: %f, Angular: %f' % (self._linear, self._angular))
        self._interface.write_line(3, 'Servo: %d' % self._servo_angle)
        self._interface.write_line(4, 'Speeds: %f, %f, %f' % self._rates)
        self._interface.write_line(5, 'Use arrow keys to move, a/z to move arm and q to exit.')
        self._interface.refresh()
